                      ...
                      foreach ($res as $pageVal) {
                        ...
                        <tr>
                          <td class="checkbox-column text-left">
                            ...
                            <label class="new-control new-checkbox checkbox-outline-info  m-auto">
                              <input type="checkbox" name="arr_ids[]" value="<?php echo $pageVal['order_id'];  ?>" class="new-control-input child-chk select-customers-info">
                              <span class="new-control-indicator"></span><span style="visibility:hidden">c</span>                            </label>                          </td>
                          <td class="left"><strong><?php echo $pageVal['invoice_number']; ?></strong><br />
                            ...
                            <strong><a href="<?php echo site_url(); ?>wps-admin/orders/vieworder/<?php echo $pageVal['order_id']; ?>" target="_blank" class="view_btn"><i class="fa fa-eye"></i> View Order</a></strong><br />

<?php
if($ordnum>=1)
{
?>
 <strong><a href="<?php echo site_url(); ?>wps-admin/orders/vieworderandupload/<?php echo $pageVal['order_id']; ?>" target="_blank" class="view_btn">
 <i style="color:#FF0000" class="fa fa-upload"></i><strong style="color:#FF0000">Upload Order to Ecomm</strong> </a> </strong>
<?php } else { ?>
 <strong><a href="#" target="_blank" class="view_btn">
 <i style="color:#009900" class="fa fa-check"></i><strong style="color:#009900">Order Uploaded</strong> </a> </strong>
<?php } ?>                          </td>
...
WHERE order_id = '" . $pageVal['order_id'] . "' GROUP BY vadmin.admin_id")->result_array();
 ...
 $ordnum = $this->db->query("SELECT vadmin.`name` AS vendor_name,vadmin.admin_id as vendor_id,sub_order_id,ecomm_awb_number FROM wps_orders_products AS ordp
LEFT JOIN wps_admin AS vadmin ON ordp.vendor_id=vadmin.admin_id
WHERE order_id = '" . $pageVal['order_id'] . "' GROUP BY vadmin.admin_id")->num_rows();
...
<?php echo $ordDetails[$i]['ecomm_awb_number']; ?></a>
...
<br /><br />
 ...
 <?php } $i++; } ?></td>
                          ...
                          <td align="left">
                            ...
                            ?>
                            <br /><br />
                            <strong>Payment Method : </strong><?php echo $pageVal['payment_method']; ?>                          </td>
                          <td class="left">
                            <?php
...
<a target="_blank" class="badge outline-badge-danger" href="https://heywansaa.com/wps-admin/orders/shipcancel/<?php echo $ordDetails[$i]['ecomm_awb_number']; ?>">
Cancel Order<br />
<?php echo $ordDetails[$i]['ecomm_awb_number']; ?>
