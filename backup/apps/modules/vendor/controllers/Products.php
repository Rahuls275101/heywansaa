...
class Products extends Admin_Controller {
  ...
  public function index($page = NULL) {
    ...
    $query="SELECT * FROM `wps_categories` ORDER BY `wps_categories`.`category_name` ASC";
    $data['category_list']=$this->product_model->get_where_array($query);
    

    
    
    $query="SELECT wps_products.*,wps_products_media.media FROM `wps_products_media` 
    left join wps_products on wps_products.products_id=wps_products_media.products_id where wps_products.vendor_id='$vendor_id'
    group by products_id order by wps_products.products_id desc";
    $data['vendor_wise_product']=$this->product_model->get_where_array($query);
  ...
  public function add() {
    ...
    $this->form_validation->set_rules('cancelation_expiry_days', 'cancelation_expiry_days', "trim|required");
   ...
   //size
      ...
      $posted_data = array(
        ...
        //   'brand'=>$this->input->post('brand'),
          ...
          'cancelation_expiry_days'=>$this->input->post('cancelation_expiry_days'),
          'vendor_id'=>$vendor_id,
          'status'=>'0',
      ...
      redirect('wps-vendor/products/index/' . $this->input->post('category_id'), '');
    }
    ...
    $data['brand']  =   $this->admin_common_model->get_where('product_brands',$where);
    
    $query="SELECT * FROM `wps_categories` ORDER BY `wps_categories`.`category_id` ASC";
    $data['category_list']=$this->product_model->get_where_array($query);
     ...
     // $this->form_validation->set_rules('browsed_image', 'Browsed Image', "trim");
      ...
      if (!empty($_FILES) && $_FILES['img4']['name'] != '') {
        ...
        if ($this->input->post('category_id') > 0) {
          ...
          redirect('wps-vendor/products/' . query_string(), '');
        }
      }
  ...
  public function view_stocks() {
    ...
    if (is_array($res)) {
      ...
      if ($this->input->post('sub') != '') {
        ...
        if ($post_error === FALSE) {
          ...
          $this->session->set_userdata(array('msg_type' => 'success'));
          $this->session->set_flashdata('success', lang('successupdate'));
          redirect('wps-vendor/products/view_stocks/' . $res['products_id'], '');
        }
      }
    ...
    {
             $keyword= $this->input->post('keywordSearch');
            $this->load->helper(array('products/product'));

            $vendor_id=$this->get_vendor_id();
            
            $query="SELECT wps_products.*,wps_products_media.media FROM `wps_products_media` left join wps_products on wps_products.products_id=wps_products_media.products_id where wps_products.vendor_id='$vendor_id' and (wps_products.product_name LIKE'%".$keyword."%') group by products_id";
            $data['vendor_wise_product']=$this->product_model->get_where_array($query);
        
        print_r($data['vendor_wise_product']);
            //Call of View
            // $this->load->view('products/view_product_list', $data);
    }
