...
class Usermodel extends Model
{
    ...
    var $table = 'admin';
    var $column_order = array(null, 'email'); //set column field database for datatable orderable
    ...
    private function _get_datatables_query()
    {
        ...
        $builder->where('admin_type', 'Admin');

        if($_POST['name_search'])
        {
        $builder->like('email', $_POST['name_search']);
        
        }

        if($_POST['status'])
        {
        $builder->where('email', $_POST['status']);
        }
