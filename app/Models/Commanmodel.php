...
class Commanmodel extends Model
{
    ...
    function getCity($state_id)
        ...
        public function getnamecategory($id, $depth = 0) {
    if ($depth > 10) return ''; // max 10 levels
    $commanmodel = new Commanmodel();
    $name = '';
    $currentCategory = $commanmodel->get_single_query('category', ['category_id' => $id]);
    if ($currentCategory) {
        $name .= $this->getnamecategory($currentCategory->parent_id, $depth + 1);
...
public function getDataFromTable($tableName, $filters = [], $orderBy = null, $limit = null, $offset = 0)
{
...
{
    // Fetch all ratings and reviews for the specified product/course
    $db = \Config\Database::connect();
    $builder = $db->table('reviews');
    $query = $builder->select('rating, user_name, message, review_date')  // Include review date
                     ->where('product_id', $course_id)
                     ->where('reviews_status', 'Active')
                     ->get();
    
    $output = '';
    $reviews_output = ''; // To hold the HTML for reviews
    $countrating_user = $query->getNumRows();

    // If there are reviews
    if ($countrating_user > 0)
    {
        $total_rating = 0;
        foreach ($query->getResult() as $row) {
            $total_rating += $row->rating;

            // Format the review date to a readable format (e.g., "April 12, 2018")
            $review_date = date("F j, Y", strtotime($row->review_date));

            // Calculate the rating width based on the individual rating
            $rating_width = ($row->rating / 5) * 100; // Convert the rating to percentage

            // Collect reviews with user name, message, and rating
            $reviews_output .= '<div class="comments">
                                    <figure class="img-thumbnail">
                                        <img src="' . base_url('assets/frontend') . '/assets/images/blog/author.jpg" alt="author" width="80" height="80">
                                    </figure>
                                        ...
                                        <div class="comment-header">
                                            ...
                                            <div class="ratings-container float-sm-right">
                                                <div class="product-ratings">
                                                    <span class="ratings" style="width:' . $rating_width . '%"></span>
                                                    <!-- End .ratings -->
                                            ...
                                            <span class="comment-by">
                                                <strong>' . htmlspecialchars($row->user_name) . '</strong> – ' . $review_date . '
                                            </span>
                                        </div>
        ...
        $average_rating = $total_rating / $countrating_user;
        $rating_round = round($average_rating * 2) / 2;  // Round to the nearest 0.5

        // Calculate the percentage rating based on the average rating
        $rating_percentage = ($average_rating / 5) * 100;  // Calculate percentage from the maximum rating (5 stars)

        // Determine the star representation based on the average rating
        $output = '';
        for ($i = 1; $i <= 5; $i++) {
            if ($i <= $rating_round) {
    ...
    {
        $outputnum = 0.00;
        $rating_percentage = 0;  // No rating percentage if there are no ratings
        $output = '<i class="fa fa-star empty"></i><i class="fa fa-star empty"></i><i class="fa fa-star empty"></i><i class="fa fa-star empty"></i><i class="fa fa-star empty"></i>';
    ...
    return array(
        ...
        'review_count' => $countrating_user  // Total review count added here
    );
}
